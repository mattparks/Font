...
static void LoadFont(const std::string &filename, Render *r)
{
	...
	r->glyphInfos = std::vector<HostGlyphInfo>(glyphCount);
...
static void CreateInstance(Render *r)
{
	...
	applicationInfo.pApplicationName = "Acid Font";
...
{
	VkShaderModule vsFont = LoadShaderModule(r->device, "Resources/Game/Shaders/Fonts2/Font.vert.spv");
	VkShaderModule fsFont = LoadShaderModule(r->device, "Resources/Game/Shaders/Fonts2/Font.frag.spv");
...
static void RenderFrame(Render *r)
{
	VkResult res = vkAcquireNextImageKHR(r->device, r->swapchain, std::numeric_limits<uint32_t>::max(),
		r->imageAvailableSemaphore, VK_NULL_HANDLE, &r->imageIndex);
...
int main(int argc, const char **args)
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Acid Font", nullptr, nullptr);
...
//	std::string filename = "Roboto-ThinItalic.ttf";
	CreateVulkanObjects("Resources/Game/Fonts/" + filename, &render);
