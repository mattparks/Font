...
const uint32_t Renderer::MAX_VISIBLE_GLYPHS = 4096;
const uint32_t Renderer::NUMBER_OF_GLYPHS = 256;
...
void CallbackKey(GLFWwindow *window, int key, int scancode, int action, int mods)
{
	...
	{
		renderer->m_ring_buffer_count = 2;
		renderer->m_canvas_scale = 3.0f;
		renderer->m_target_canvas_scale = 3.0f;
		renderer->m_canvas_offset = acid::Vector2();
	}
}
...
{
	auto renderer = (Renderer *)(glfwGetWindowUserPointer(window));
	renderer->m_target_canvas_scale *= powf(1.3f, (float)yoffset);
}
...
uint32_t Renderer::align_uint32(const uint32_t &value, const uint32_t &alignment)
{
...
void Renderer::find_queue_families()
{
...
void Renderer::choose_surface_format()
{
...
void Renderer::choose_present_mode()
{
...
void Renderer::append_text(float x, float y, float scale, const std::string &text)
{
	for (char c : text)
	{
...
void Renderer::record_current_command_buffer()
{
	//printf("%d\n", m_ring_buffer_index);
	VkCommandBuffer cmd_buf = m_command_buffers[m_ring_buffer_index];
...
void Renderer::create_command_buffer_fences()
{
...
VkShaderModule Renderer::load_shader_module(VkDevice device, const char *path)
{
...
uint32_t Renderer::find_memory_type(uint32_t type_bits, VkMemoryPropertyFlags flags)
{
...
VkDeviceMemory Renderer::alloc_required_memory(VkMemoryRequirements *req, VkMemoryPropertyFlags flags)
{
...
void Renderer::create_buffer_with_memory(VkBufferCreateInfo *ci, VkMemoryPropertyFlags flags, VkDeviceMemory *memory,
                                         VkBuffer *buffer)
{
...
VkCommandBuffer Renderer::begin_one_time_cmdbuf()
{
...
void Renderer::end_one_time_cmdbuf(VkCommandBuffer cmd_buffer)
{
...
void Renderer::stage_buffer(VkBuffer buffer, void *data, size_t size)
{
...
void Renderer::create_storage_buffer()
{
	load_font();
...
void Renderer::create_instance_buffer()
{
...
void Renderer::create_swap_chain_objects()
{
	create_image_views();
	create_render_pass();
	create_pipeline();
...
void Renderer::create_vulkan_objects()
{
	create_instance();
	create_surface();
...
void Renderer::destroy_swap_chain_objects()
{
...
void Renderer::destroy_vulkan_objects()
{
